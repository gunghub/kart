...
class PyTuxJT:
    ...
    def __init__(self, screen_width=128, screen_height=96):
        assert PyTuxJT._singleton is None, "Cannot create more than one pytux object"
        PyTuxJT._singleton = self
    ...
    def rollout_jt2(self, track, controller, planner=None, max_frames=1000, verbose=False, data_callback=None,device="cpu"):
        """
        Play a level (track) for a single round.
        :param track: Name of the track
        :param controller: low-level controller, see controller.py
        ...
        for t in range(max_frames):
            ...
            if planner:
                image = np.array(self.k.render_data[0].image)
                aim_point = planner(TF.to_tensor(image)[None],device=device).squeeze(0).cpu().detach().numpy()
...
class PyTux:
    ...
    def rollout(self, track, controller, planner=None, max_frames=1000, verbose=False, data_callback=None, device="cpu"):
        """
        Play a level (track) for a single round.
        :param track: Name of the track
        :param controller: low-level controller, see controller.py
...
def generate_trainset(pytux, track, control, output=DATASET_PATH, n_images=10000, steps_per_track=20000, aim_noise=0.1, vel_noise=5, verbose=False):
